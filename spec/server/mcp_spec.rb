...
RSpec.describe "MCP connector" do
  ...
  # The browser-facing half of this flow — logging in, granting consent, saving a
  # caption from the phone page — goes through the same CSRF gate as the rest of
  # the site. The machine half does not, and the posts below that send a raw JSON
  # body (/register, /mcp) deliberately stay tokenless to prove it.
  include CsrfHelpers
  ...
  def authorize_and_fetch_token(account, scopes: %w[hikes:read hikes:write])
    ...
    # Granting an outside app access to an account is a state change, so the
    # consent form carries a token and the POST below uses that one rather than
    # a freshly minted one -- if the form ever stops emitting it, this breaks.
  ...
  describe "authorization" do
    ...
    # The site checks a form token on every other POST. These four endpoints sit
    # on either side of that line and the line is easy to move by accident, so
    # each one is pinned: an API client has no page to take a token from, and the
    # consent form is a browser form like any other.
    it "exempts the machine endpoints from the form-token gate and keeps the consent form behind it" do
      account = create_account
      # Registers through the tokenless raw-JSON POST /register, which asserts 201.
      token_payload = authorize_and_fetch_token(account)
      client_id = WentHiking.db[:oauth_applications].first[:client_id]

      post_without_csrf "/token", {
        "grant_type" => "refresh_token",
        "refresh_token" => token_payload["refresh_token"],
        "client_id" => client_id
      }
      expect(last_response.status).to eq(200), last_response.body
      refreshed = JSON.parse(last_response.body).fetch("access_token")

      # RFC 7009 revocation as a real client sends it: form-encoded body, JSON
      # accepted back, and no browser session at all.
      clear_cookies
      post_without_csrf "/revoke", {"token" => refreshed, "client_id" => client_id}, {"HTTP_ACCEPT" => "application/json"}
      expect(last_response.status).to eq(200), last_response.body
      mcp_request(refreshed, {jsonrpc: "2.0", id: 1, method: "tools/list"})
      expect(last_response.status).to eq(401)

      _, challenge = pkce_pair
      post_without_csrf "/authorize", {
        "client_id" => client_id,
        "redirect_uri" => redirect_uri,
        "response_type" => "code",
        "code_challenge" => challenge,
        "code_challenge_method" => "S256",
        "scope" => %w[hikes:read hikes:write]
      }
